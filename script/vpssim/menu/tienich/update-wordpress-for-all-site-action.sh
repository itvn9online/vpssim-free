				...
				if [ -f $get_d/wp-config.php ] && [ -f $get_d/wp-settings.php ] && [ -f $get_d/wp-blog-header.php ] && [ -d $get_d/wp-admin ] && [ -d $get_d/wp-content ] && [ -d $get_d/wp-includes ]; then
					...
					if [ -f $get_d/index.php ]; then
						...
						if grep -q base64_decode "$checkFile"; then
							...
							curl --data "source=index&path="$get_d https://cloud.echbay.com/backups/has_malware
						fi
					fi
					# file wp-login php la noi no thu thap thong tin dang nhap cua nguoi dung
					if [ -f $get_d/wp-login.php ]; then
						checkFile=$get_d/wp-login.php
						...
						if grep -q logo.gif "$checkFile"; then
							# gui canh bao ve telegram
							curl --data "source=wp-login&path="$get_d https://cloud.echbay.com/backups/has_malware
						fi
					fi
					# file logo.gif nay cung do virus tao ra
					if [ -f $get_d/logo.gif ]; then
						# gui canh bao ve telegram
						curl --data "source=logogif&path="$get_d https://cloud.echbay.com/backups/has_malware
					fi
