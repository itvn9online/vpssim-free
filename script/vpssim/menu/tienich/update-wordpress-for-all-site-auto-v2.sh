...
/usr/bin/echo "root_dir=\"/www/wwwroot\"" >> /tmp/server_wp_all_update
...
#
/usr/bin/echo "chmodUser=\"www\"" >> /tmp/server_wp_all_update
/usr/bin/echo "chmodAllUser=\"www\"" >> /tmp/server_wp_all_update
...
if [ -d /etc/vpssim/menu/tienich ]; then
  ...
  if [ ! -f /etc/vpssim/menu/tienich/update-wordpress-for-all-site ]; then
    cd ~
    cd /etc/vpssim/menu/tienich
    /usr/bin/curl -sO https://raw.githubusercontent.com/itvn9online/vpssim-free/master/script/vpssim/menu/tienich/update-wordpress-for-all-site
    cd ~
  fi
