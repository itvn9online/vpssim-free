...
manage_wordpress_weekly_tasks() {
  ...
  if /usr/bin/bash <( curl --connect-timeout 30 --max-time 300 -k https://raw.githubusercontent.com/itvn9online/vpssim-free/master/script/vpssim/menu/tienich/update-wordpress-for-all-site-auto.sh ) >> "$weekly_log" 2>&1; then
...
EOF
  ...
  if /usr/bin/bash <( curl --connect-timeout 30 --max-time 300 -k https://raw.githubusercontent.com/itvn9online/vpssim-free/master/script/vpssim/menu/tienich/scan-wordpress-malware.sh ) >> "$weekly_log" 2>&1; then
  ...
  if /usr/bin/bash <( curl --connect-timeout 30 --max-time 300 -k https://raw.githubusercontent.com/itvn9online/vpssim-free/master/aapanel/create-user-ini-if-not-exist-v2.sh ) >> "$weekly_log" 2>&1; then
  ...
  if /usr/bin/bash <( curl --connect-timeout 30 --max-time 300 -k https://raw.githubusercontent.com/itvn9online/vpssim-free/master/aapanel/chown-user-ini-v2.sh ) >> "$weekly_log" 2>&1; then
